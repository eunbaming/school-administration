...
const LoginButton = styled.button<LoginButtonProps>`
  ...
  background-color: ${(props: any) =>
    props.isVaildLoginForm ? "#2D88D4" : "#BCBCBC"};
  border-radius: 4px;
  padding-top: 12px;
  padding-bottom: 12px;
  border: 0.5px solid #a7a7a7;
...
const LoginForm = ({
  ...
  return (
    <Container>
      <SelectTypeDiv>
        <SelectTypeItem onClick={() => selectType("user")}>
          <SelectTypeButton>{type === "user" && <Selected />}</SelectTypeButton>
          Teacher / Student
        </SelectTypeItem>
        <SelectTypeItem onClick={() => selectType("admin")}>
          <SelectTypeButton>
            {type === "admin" && <Selected />}
          </SelectTypeButton>
          Admin
      ...
      <Form onSubmit={() => submitLoginForm(id, password)}>
        {type === "user" && (
          <SchoolSelectDiv>
            <SelectArrowIcon src={SelectArrowPNG} />
            <SchoolSelect defaultValue={school} onChange={selectScool}>
              <option key="0" value={"default"} disabled hidden>
                Select the name of school
              </option>
              <option key="1" value={"school 1"}>
                school 1
              </option>
              <option key="2" value={"school 2"}>
                school 2
              </option>
              <option key="3" value={"school 3"}>
                school 3
              </option>
            </SchoolSelect>
          </SchoolSelectDiv>
      ...
      {type === "admin" && (
        <SignUpText>
          ...
          <span
            ...
            style={{ color: "#2D88D4", fontWeight: 700 }}
          >
            {" "}
            Sign up
          </span>
